...
get_disk() {
    ...
    printf "%s %s/%s (%s)" "$PREFIX_FILESYSTEM" "$STOUSED" "$STOTOT" "$STOPER"
}
...
dwm_resources() {
    ...
    printf "%s" "$SEP1"
    if [ "$IDENTIFIER" = "unicode" ]; then
        printf "💻 RAM %s/%s | CPU %s " "$MEMUSED" "$MEMTOT" "$CPU"
        printf "%s %s/%s (%s)" "$PREFIX_FILESYSTEM" "$STOUSED" "$STOTOT" "$STOPER"
    else
...
dwm_spotify() {
    ...
    if [ "$PLAYER" = "spotify" ] || [ "$PLAYER" = "spotifyd" ] || [ "$PLAYER" = "ncspot" ]; then
        ...
        printf "$powerline_h %s %s- %s" "$STATUS" "${ARTIST:0:14}" "${TRACK:0:9}..."
...
while true; do
    ((interval++))
    xsetroot -name "^c#F5E4CE^$(dwm_spotify) $(process) $(get_cputemp)^c#F5E4CE^ $(memory) $(get_disk) $(batteries_t480) $(updates)^c#F5E4CE^ $(dwm_alsa) $(datetime) $(dwm_countdown) $(sys_tray_space)"
    sleep 2
done
