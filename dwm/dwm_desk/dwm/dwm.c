...
void setup(void)
{
	...
	// scheme[LENGTH(colors)] = drw_scm_create(drw, colors, 3);
	scheme[LENGTH(colors)] = drw_scm_create(drw, (const char **)colors[0], 4);
	for (i = 0; i < LENGTH(colors); i++)
		scheme[i] = drw_scm_create(drw, (const char **)colors[i], 4);
